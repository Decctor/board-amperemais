...
export const AppSubscriptionPlans: {
  ...
  ESSENCIAL: {
    name: "ESSENCIAL",
    description:
      "Comece hoje. Cashback + PDV em tablet + campanhas básicas. Sem integração obrigatória.",
...
export function getOrganizationAccessToRoute({
  organizationPlan,
  path,
}: {
  organizationPlan: keyof typeof AppSubscriptionPlans;
  path: string;
}) {
...
export const AppRoutes = [
  ...
  {
    path: "/dashboard/commercial/campaigns-flows",
    title: "Fluxos de Campanha",
    description: "Painel de acompanhamento e gestão dos fluxos de campanha.",
  },
  {
...
export function checkSubscriptionStatus({
  stripeStatus,
  stripeStatusUltimaAlteracao,
  trialPeriodStart,
  trialPeriodEnd,
}: TCheckSubscriptionStatus) {
  ...
  if (stripeStatus === "past_due") {
    ...
    const daysSinceChange = Math.floor(
      (Date.now() - new Date(stripeStatusUltimaAlteracao).getTime()) / (1000 * 60 * 60 * 24),
    );
