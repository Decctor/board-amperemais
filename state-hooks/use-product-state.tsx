...
export const useProductState = ({ initialState }: UseProductStateProps = {}) => {
  ...
  const updateProduct = useCallback(
    (updates: Partial<Omit<TProductState["product"], "imagemCapaHolder">>) => {
      setState((prev) => ({
        ...prev,
        product: {
          ...prev.product,
  ...
  const updateProductImageHolder = useCallback(
    (holder: Partial<TProductState["product"]["imagemCapaHolder"]>) => {
      setState((prev) => ({
        ...prev,
        product: {
          ...prev.product,
  ...
  const updateProductVariant = useCallback(
    (
      index: number,
      updates: Partial<Omit<TProductVariantState, "imagemCapaHolder" | "addOns">>,
    ) => {
      setState((prev) => ({
        ...prev,
  ...
  const updateProductVariantImageHolder = useCallback(
    (variantIndex: number, holder: Partial<TProductVariantState["imagemCapaHolder"]>) => {
      setState((prev) => ({
        ...prev,
        productVariants: prev.productVariants.map((variant, i) =>
          i === variantIndex
  ...
  const updateProductAddOn = useCallback(
    (index: number, updates: Partial<Omit<TProductAddOnState, "opcoes">>) => {
      setState((prev) => ({
        ...prev,
  ...
  const removeProductAddOn = useCallback((index: number) => {
    setState((prev) => {
      ...
      if (addOn?.id) {
        return {
          ...prev,
          productAddOns: prev.productAddOns.map((a, i) =>
            i === index ? { ...a, deletar: true } : a,
          ),
        };
      }
  ...
  const addProductAddOnOption = useCallback(
    (addOnIndex: number, option: TProductAddOnOptionState) => {
      setState((prev) => ({
        ...prev,
        productAddOns: prev.productAddOns.map((addOn, i) =>
          i === addOnIndex
  ...
  const updateProductAddOnOption = useCallback(
    (addOnIndex: number, optionIndex: number, updates: Partial<TProductAddOnOptionState>) => {
      setState((prev) => ({
        ...prev,
        productAddOns: prev.productAddOns.map((addOn, i) =>
          i === addOnIndex
            ? {
                ...addOn,
                opcoes: addOn.opcoes.map((option, j) =>
  ...
  const updateVariantAddOn = useCallback(
    (
      variantIndex: number,
      addOnIndex: number,
      updates: Partial<Omit<TVariantAddOnState, "opcoes">>,
    ) => {
      setState((prev) => ({
        ...prev,
        productVariants: prev.productVariants.map((variant, i) =>
  ...
  const addVariantAddOnOption = useCallback(
    (variantIndex: number, addOnIndex: number, option: TProductAddOnOptionState) => {
      setState((prev) => ({
        ...prev,
        productVariants: prev.productVariants.map((variant, i) =>
          i === variantIndex
  ...
  const updateVariantAddOnOption = useCallback(
    (
      ...
      addOnIndex: number,
      optionIndex: number,
      updates: Partial<TProductAddOnOptionState>,
    ) => {
      setState((prev) => ({
        ...prev,
        productVariants: prev.productVariants.map((variant, i) =>
            ...
            ? {
                ...
                addOns: variant.addOns.map((addOn, j) =>
                  j === addOnIndex
                    ? {
                        ...addOn,
                        opcoes: addOn.opcoes.map((option, k) =>
                          k === optionIndex ? { ...option, ...updates } : option,
                        ),
                      }
                    : addOn,
                ),
  ...
  const removeVariantAddOnOption = useCallback(
    (variantIndex: number, addOnIndex: number, optionIndex: number) => {
      setState((prev) => ({
        ...prev,
        productVariants: prev.productVariants.map((variant, i) => {
          if (i !== variantIndex) return variant;
          ...
          return {
            ...
            addOns: variant.addOns.map((addOn, j) => {
              ...
              if (option?.id) {
                return {
                  ...addOn,
                  opcoes: addOn.opcoes.map((o, k) =>
                    k === optionIndex ? { ...o, deletar: true } : o,
                  ),
                };
              }
              // Se é nova (sem id), remove da lista
