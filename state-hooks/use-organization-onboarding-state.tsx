...
const OrganizationOnboardingStateSchema = z.object({
	...
	subscription: z
		...
		.optional()
		.nullable(),
	indicadorCodigo: z.string({ invalid_type_error: "Tipo nÃ£o vÃ¡lido para o cÃ³digo de indicaÃ§Ã£o." }).optional().nullable(),
	...
	const start: TOrganizationOnboardingState = useMemo(
		() => ({
			...
			organization: {
				...
				origemDadosPadrao: initialState?.organization?.origemDadosPadrao ?? "RECEPTOR",
				fiscalEmissaoAutomatica: initialState?.organization?.fiscalEmissaoAutomatica ?? false,
				fiscalProvedor: initialState?.organization?.fiscalProvedor ?? null,
				fiscalConfiguracao: initialState?.organization?.fiscalConfiguracao ?? null,
				...
				localizacaoNumero: initialState?.organization?.localizacaoNumero ?? null,
				localizacaoComplemento: initialState?.organization?.localizacaoComplemento ?? null,
				...
				corPrimaria: initialState?.organization?.corPrimaria ?? null,
				corPrimariaForeground: initialState?.organization?.corPrimariaForeground ?? null,
				corSecundaria: initialState?.organization?.corSecundaria ?? null,
				corSecundariaForeground: initialState?.organization?.corSecundariaForeground ?? null,
				integracaoTipo: initialState?.organization?.integracaoTipo ?? null,
				integracaoConfiguracao: initialState?.organization?.integracaoConfiguracao ?? null,
				integracaoDataUltimaSincronizacao: initialState?.organization?.integracaoDataUltimaSincronizacao ?? null,
				logoUrl: initialState?.organization?.logoUrl ?? null,
				poiQrCodeKioskDataUrl: initialState?.organization?.poiQrCodeKioskDataUrl ?? null,
				poiQrCodeMobileDataUrl: initialState?.organization?.poiQrCodeMobileDataUrl ?? null,
			},
			...
			indicadorCodigo: initialState?.indicadorCodigo ?? null,
