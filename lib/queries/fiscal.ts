import type {
  TGetFiscalDocumentsInput,
  TGetFiscalDocumentsOutput,
...
async function fetchFiscalSettings() {
  const { data } = await axios.get<TGetFiscalSettingsOutput>(
    "/api/fiscal/settings",
  );
  return data.data;
}
...
async function fetchFiscalDocuments({
  page,
  search,
  statusInterno,
  ...
  if (statusInterno && statusInterno.length > 0)
    searchParams.set("statusInterno", statusInterno.join(","));
  const { data } = await axios.get<TGetFiscalDocumentsOutput>(
    `/api/fiscal/documents?${searchParams.toString()}`,
  );
  const result = data.data.default;
  if (!result)
    throw new Error("Oops, houve um erro ao buscar os documentos fiscais.");
  return result;
}
async function fetchFiscalDocumentById({ documentId }: { documentId: string }) {
  ...
  if (!result)
    throw new Error("Oops, houve um erro ao buscar o documento fiscal.");
  return result;
}
...
async function fetchFiscalSeriesById({ id }: { id: string }) {
  const { data } = await axios.get<TGetFiscalSeriesOutput>(
    `/api/fiscal/series?id=${id}`,
  );
  const byIdData = data.data.byId;
  if (!byIdData)
    throw new Error("Oops, houve um erro ao buscar a serie fiscal.");
  return byIdData;
}
...
async function fetchFiscalSeries() {
  const { data } =
    await axios.get<TGetFiscalSeriesOutput>("/api/fiscal/series");
  const defaultData = data.data.default;
  if (!defaultData)
    throw new Error("Oops, houve um erro ao buscar as series fiscais.");
  return defaultData;
}
...
async function fetchFiscalOperationProfileById({ id }: { id: string }) {
  const { data } = await axios.get<TGetFiscalOperationProfilesOutput>(
    `/api/fiscal/operation-profiles?id=${id}`,
  );
...
async function fetchFiscalOperationProfiles() {
  const { data } = await axios.get<TGetFiscalOperationProfilesOutput>(
    "/api/fiscal/operation-profiles",
  );
  const defaultData = data.data.default;
  if (!defaultData)
    throw new Error(
      "Oops, houve um erro ao buscar os perfis de operação fiscal.",
    );
  return defaultData;
}
...
async function fetchFiscalTaxGroups() {
  const { data } = await axios.get<TGetFiscalTaxGroupsOutput>(
    "/api/fiscal/tax-groups",
  );
  const defaultData = data.data.default;
  if (!defaultData)
    throw new Error("Oops, houve um erro ao buscar os grupos tributarios.");
  return defaultData;
}
...
async function fetchFiscalTaxGroupById({ id }: { id: string }) {
  ...
  const { data } = await axios.get<TGetFiscalTaxGroupsOutput>(
    `/api/fiscal/tax-groups?${searchParams.toString()}`,
  );
  const result = data.data.byId;
  if (!result)
    throw new Error("Oops, houve um erro ao buscar o grupo tributario.");
  return result;
}
...
async function fetchFiscalInboundDocuments({ page }: { page: number }) {
  ...
  const { data } = await axios.get<TGetInboundDocumentsOutput>(
    `/api/fiscal/inbound?${searchParams.toString()}`,
  );
  const result = data.data.default;
  if (!result)
    throw new Error("Oops, houve um erro ao buscar as notas recebidas.");
  return result;
}
