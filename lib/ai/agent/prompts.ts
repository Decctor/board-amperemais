...
export function buildAgentSystemPrompt({
	...
	const partes: string[] = [instrucoes.trim()];

	partes.push(`## Regras do canal (WhatsApp)
...
- Nunca peça senha, dados de cartão ou documentos.`);
	...
	const regrasCondicionais: string[] = [];
	...
	partes.push(`## Como usar suas ferramentas\n${regrasCondicionais.join("\n")}`);

	const ferramentas = getEnabledAgentTools(capacidades);
	if (ferramentas.length > 0) {
		partes.push(`## Ferramentas disponíveis\n${ferramentas.map((ferramenta) => `- ${ferramenta.name}`).join("\n")}`);
	} else {
