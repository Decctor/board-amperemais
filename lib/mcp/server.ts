...
function buildInstructions(actor: TAgentActorContext) {
	const base = [
		...
		"Para cabeçalhos de imagem, use `upload_message_template_media` com `conteudoUrl` apontando para uma imagem pública https: o servidor baixa os bytes e devolve o `conteudoMidiaCaminho` já validado. NÃO converta para base64 uma imagem que já tem URL — gerar base64 é lento e estoura timeout; reserve `conteudoBase64` para arquivos pequenos (até ~200 KB) que só existem no seu ambiente. `create_message_template_media_upload` (URL assinada + `complete_message_template_media_upload`) é o último recurso, e exige que você mesmo alcance o host de armazenamento, o que nem todo ambiente permite. Esse caminho é opaco; não construa nem altere caminhos manualmente.",
	];
