...
export default function ProductCard({ product, variant = "compact" }: ProductCardProps) {
	...
	const lowestPrice = hasVariants
		? Math.min(...product.variantes.map((v) => v.precoVenda ?? 0))
	...
	if (variant === "full") {
		return (
			<div
				className="group flex gap-3 p-3 rounded-xl border bg-card hover:bg-accent/50 transition-colors cursor-pointer"
				onClick={handleClick}
			>
				...
				<div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
					<div>
						...
						{product.grupo && (
							<p className="text-xs text-muted-foreground mt-0.5">{product.grupo}</p>
						)}
					</div>
