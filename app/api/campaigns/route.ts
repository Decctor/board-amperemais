...
function validateRecurrentCampaign(campaign: z.infer<typeof CampaignSchema>) {
  ...
  if (!campaign.recorrenciaTipo) {
    throw new createHttpError.BadRequest(
      "Selecione a frequência de recorrência (DIARIO, SEMANAL ou MENSAL).",
    );
  }
  ...
  if (campaign.recorrenciaTipo === "SEMANAL") {
    if (!campaign.recorrenciaDiasSemana) {
      throw new createHttpError.BadRequest(
        "Selecione pelo menos um dia da semana para a campanha recorrente semanal.",
      );
    }
    const dias: number[] = JSON.parse(campaign.recorrenciaDiasSemana);
    if (!Array.isArray(dias) || dias.length === 0 || dias.some((d) => d < 0 || d > 6)) {
      throw new createHttpError.BadRequest(
        "Dias da semana inválidos. Use valores entre 0 (Domingo) e 6 (Sábado).",
      );
    }
  }
  ...
  if (campaign.recorrenciaTipo === "MENSAL") {
    if (!campaign.recorrenciaDiasMes) {
      throw new createHttpError.BadRequest(
        "Selecione pelo menos um dia do mês para a campanha recorrente mensal.",
      );
    }
...
function validateCashbackExpiringTrigger(campaign: z.infer<typeof CampaignSchema>) {
  ...
  if (
    ...
    throw new createHttpError.BadRequest(
      "Informe uma antecedência válida para cashback expirando.",
    );
  }
}
...
async function createCampaign({
  ...
  if (!userOrgId)
    throw new createHttpError.Unauthorized(
      "Você precisa estar vinculado a uma organização para acessar esse recurso.",
    );
  ...
  if (input.campaign.cashbackGeracaoAtivo) {
    if (!input.campaign.cashbackGeracaoTipo) {
      throw new createHttpError.BadRequest(
        "Selecione o tipo de geração de cashback (FIXO ou PERCENTUAL).",
      );
    }
  ...
  if (!insertedCampaignId)
    throw new createHttpError.InternalServerError(
      "Oops, houve um erro desconhecido ao criar campanha.",
    );
...
async function getCampaigns({
  ...
  if ("id" in input && input.id) {
    ...
    const campaign = await db.query.campaigns.findFirst({
      where: (fields, { and, eq }) =>
        and(eq(fields.id, campaignId), eq(fields.organizacaoId, userOrgId)),
      with: {
        segmentacoes: true,
      },
    });
  ...
  if (input.triggerTypes && input.triggerTypes.length > 0) {
    conditions.push(
      inArray(campaigns.gatilhoTipo, input.triggerTypes as TCampaignTriggerTypeEnum[]),
    );
  }
  ...
  if (input.statsPeriodBefore)
    interactionsConditions.push(lte(interactions.dataInsercao, input.statsPeriodBefore));
  ...
  const interactionsStats = await db
    .select({
      ...
      deliveredCount: sum(
        sql<number>`CASE WHEN ${interactions.statusEnvio} IN ('ENTREGUE', 'LIDO') THEN 1 ELSE 0 END`,
      ),
    })
    .from(interactions)
  ...
  const conversionsConditions = [
    eq(campaignConversions.organizacaoId, userOrgId),
    inArray(campaignConversions.campanhaId, campaignIds),
  ];
  if (input.statsPeriodAfter)
    conversionsConditions.push(gte(campaignConversions.dataConversao, input.statsPeriodAfter));
  if (input.statsPeriodBefore)
    conversionsConditions.push(lte(campaignConversions.dataConversao, input.statsPeriodBefore));
...
export type TGetCampaignsOutputDefault = Exclude<
  TGetCampaignsOutput["data"]["default"],
  undefined | null
>;
export type TGetCampaignsOutputById = Exclude<
  TGetCampaignsOutput["data"]["byId"],
  undefined | null
>;
...
const getCampaignsRoute = async (request: NextRequest) => {
  ...
  const input = GetCampaignsInputSchema.parse({
    ...
    actionWhatsappOnly:
      actionWhatsappOnlyParam === null ? undefined : actionWhatsappOnlyParam === "true",
    cashbackGenerationOnly:
      cashbackGenerationOnlyParam === null
        ? undefined
        : cashbackGenerationOnlyParam === "true",
...
async function updateCampaign({
  ...
  return await db.transaction(async (trx) => {
    ...
    if (!updatedCampaignId)
      throw new createHttpError.InternalServerError(
        "Oops, houve um erro desconhecido ao atualizar campanha.",
      );

    console.log(
      "[INFO] [UPDATE-CAMPAIGN] Campaign updated successfully, starting to process segmentations...",
    );
