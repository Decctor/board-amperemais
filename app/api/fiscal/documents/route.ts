...
export type TGetFiscalDocumentsInput = z.infer<
  typeof GetFiscalDocumentsInputSchema
>;

async function getFiscalDocuments({
  input,
}: {
  input: TGetFiscalDocumentsInput;
}) {
  const { session, orgId } = await requireOrgSession();

  const userHasFiscalViewPermission =
    session.membership?.permissoes.fiscal.visualizar;
  if (!userHasFiscalViewPermission)
    throw new createHttpError.Forbidden(
      "Oops, você não possui permissão para visualizar o módulo fiscal.",
    );
  ...
  if (input.documentId) {
    ...
    if (!document || document.organizacaoId !== orgId)
      throw new createHttpError.NotFound("Documento fiscal não encontrado.");
    const events = await listFiscalDocumentEvents({
  ...
  const result = await listFiscalDocuments({
    organizacaoId: orgId,
    page: input.page,
    search: input.search,
...
export type TGetFiscalDocumentsOutput = Awaited<
  ReturnType<typeof getFiscalDocuments>
>;
export type TGetFiscalDocumentsOutputById = NonNullable<
  TGetFiscalDocumentsOutput["data"]["byId"]
>;
export type TGetFiscalDocumentsOutputDefault = NonNullable<
  TGetFiscalDocumentsOutput["data"]["default"]
>;
  ...
  classificacaoPresencialExcepcional: z
    .object({
      ...
      justificativa: z
        .string({
          required_error:
            "Justificativa da classificação presencial não informada.",
          invalid_type_error:
            "Tipo não válido para a justificativa da classificação presencial.",
        })
        .trim()
        .min(
          EXCEPTIONAL_PRESENCE_JUSTIFICATION_MIN_LENGTH,
...
export type TEmitFiscalDocumentInput = z.infer<
  typeof EmitFiscalDocumentInputSchema
>;

async function createFiscalDocument({
  input,
}: {
  input: TEmitFiscalDocumentInput;
}) {
  const { session, orgId } = await requireOrgSession();
  const userHasFiscalEmitPermission =
    session.membership?.permissoes.fiscal.emitir;
  if (!userHasFiscalEmitPermission)
    throw new createHttpError.Forbidden(
      "Oops, você não possui permissão para emitir documentos fiscais.",
    );

  const saleBelongsToOrg = await db.query.sales.findFirst({
    where: (fields, { and, eq }) =>
      and(eq(fields.id, input.vendaId), eq(fields.organizacaoId, orgId)),
    with: {
  ...
  if (!saleBelongsToOrg)
    throw new createHttpError.NotFound(
      "Venda não encontrada para emissão fiscal.",
    );
  ...
  if (input.classificacaoPresencialExcepcional) {
    if (saleBelongsToOrg.statusVenda !== "CONFIRMADA") {
      throw new createHttpError.BadRequest(
        "Apenas vendas confirmadas podem usar a classificação presencial excepcional.",
      );
    }
    const restriction = getExceptionalPresenceRestriction({
      ...
      podeConfigurarFiscal:
        !!session.user.admin ||
        !!session.membership?.permissoes.fiscal.configurar,
      entregaModalidade: saleBelongsToOrg.entregaModalidade,
    });
    if (restriction?.code === "MISSING_PERMISSION")
      throw new createHttpError.Forbidden(restriction.message);
    if (restriction) throw new createHttpError.BadRequest(restriction.message);
  }
  if (!input.tipo) {
    if (saleBelongsToOrg.statusVenda !== "CONFIRMADA") {
      throw new createHttpError.BadRequest(
        "Apenas vendas confirmadas podem emitir nota fiscal.",
      );
    }
    ...
    if (hasActiveFiscalDocument) {
      throw new createHttpError.Conflict(
        "Esta venda já possui um documento fiscal ativo.",
      );
    }
  }
  const tipo =
    ...
    (await resolveEmissionDocumentType({
      organizacaoId: orgId,
      operacaoPadraoNfeId:
        fiscalOrganization?.fiscalConfiguracao?.operacaoPadraoPorTipo?.NFE ??
        null,
      signals: {
...
export type TCreateFiscalDocumentOutput = Awaited<
  ReturnType<typeof createFiscalDocument>
>;
