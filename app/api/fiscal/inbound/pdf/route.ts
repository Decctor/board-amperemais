...
import { getInboundDocumentPdf } from "@/lib/fiscal/inbound";
import createHttpError from "http-errors";
import { NextRequest, NextResponse } from "next/server";

async function getInboundPdfRoute(request: NextRequest) {
	...
	const { buffer, contentType } = await getInboundDocumentPdf({ organizationId: orgId, inboundId });
	return new NextResponse(buffer as ArrayBuffer, { status: 200, headers: { "Content-Type": contentType } });
}

export const GET = appApiHandler({ GET: getInboundPdfRoute });
