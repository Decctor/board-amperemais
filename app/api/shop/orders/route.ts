...
import { SaleAttendanceStatusEnum, SaleStatusEnum } from "@/schemas/enums";
...
const GetShopOrdersInputSchema = z.object({
	...
	statusAtendimento: SaleAttendanceStatusEnum.optional().nullable(),
...
async function getShopOrdersRoute(request: NextRequest) {
	...
	const input = GetShopOrdersInputSchema.parse({
		...
		statusAtendimento: searchParams.get("statusAtendimento") ?? undefined,
		search: searchParams.get("search") ?? undefined,
	});
	const pageSize = 20;
	const page = Math.max(1, input.page || 1);

	const conditions = [eq(sales.organizacaoId, orgId), eq(sales.canal, "SHOP"), eq(sales.statusVenda, input.status ?? "ORCAMENTO")];
	if (input.statusAtendimento) {
		conditions.push(eq(sales.statusAtendimento, input.statusAtendimento));
	}
	if (input.search) {
