...
const CreateAndConfirmSaleInputSchema = z.object({
  clienteId: z
    ...
    .nullable(),
  vendedorId: z
    .string({ invalid_type_error: "Tipo não válido para ID do vendedor." })
    .optional()
    .nullable(),
  vendedorNome: z
    .string({ invalid_type_error: "Tipo não válido para nome do vendedor." })
    .optional()
    .nullable(),
  entregaModalidade: z.enum(["PRESENCIAL", "RETIRADA", "ENTREGA", "COMANDA"]).optional().nullable(),
  entregaLocalizacaoId: z
    .string({ invalid_type_error: "Tipo não válido para ID da localização." })
    .optional()
    .nullable(),
  comandaNumero: z
    .string({ invalid_type_error: "Tipo não válido para comanda." })
    .optional()
    .nullable(),
  observacoes: z
    ...
    .nullable(),
  descontosTotal: z
    .number({ invalid_type_error: "Tipo não válido para desconto." })
    .optional()
    .nullable(),
  acrescimosTotal: z
    .number({ invalid_type_error: "Tipo não válido para acréscimo." })
    .optional()
    .nullable(),
  ...
  cashbackResgate: z
    ...
    .default(0),
  cashbackProgramaId: z
    .string({ invalid_type_error: "Tipo não válido para ID do programa de cashback." })
    .optional()
    .nullable(),
...
async function createAndConfirmSale({
  ...
  const custoTotal = input.itens.reduce((sum, item) => {
    const custo = item.produtoVarianteId
  ...
  const saleId = await db.transaction(async (tx) => {
    const insertedSale = await tx
      ...
      .values({
        ...
        valorTotal,
        descontosTotal:
          (input.descontosTotal ?? descontosTotalItens) > 0
            ? (input.descontosTotal ?? descontosTotalItens)
            : null,
  ...
  const confirmation = await processSaleConfirmation({
    ...
    accountingEntryDebitAccountId: DEFAULT_POS_CONTA_DEBITO_ID,
    accountingEntryCreditAccountId: DEFAULT_POS_CONTA_CREDITO_ID,
  });
