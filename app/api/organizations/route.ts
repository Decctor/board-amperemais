...
async function createOrganization({
  ...
  console.log(
    "[INFO] [CREATE_ORGANIZATION] Starting the organization onboarding conclusion process:",
  ...
  const insertedOrgId = await db.transaction(async (tx) => {
    ...
    if (!createdOrgId)
      throw new createHttpError.InternalServerError(
        "Oops, houve um erro desconhecido ao criar organização.",
      );
    console.log(
      "[INFO] [CREATE_ORGANIZATION] Organization created successfully with ID:",
      createdOrgId,
    );
  ...
  try {
    ...
    console.error(
      "[WARN] [CREATE_ORGANIZATION] Falha ao capturar evento onboarding_completed:",
      error,
    );
  }
  ...
  if (!subscription || subscription === "FREE-TRIAL") {
    ...
    void notifyInternalsOnNewOrganization({
      ...
      console.error("[WARN] [CREATE_ORGANIZATION] Falha ao notificar fundadores:", err),
    );

    void welcomeOrganizationOwnerOnOnboarding({ orgOwner: sessionUser }).catch((err) =>
      console.error(
        "[WARN] [CREATE_ORGANIZATION] Falha ao enviar boas-vindas ao dono da organização:",
        err,
      ),
    );
    ...
    try {
      ...
      console.error(
        "[WARN] [CREATE_ORGANIZATION] Falha ao capturar evento onboarding_completed_with_trial:",
        error,
      );
    }
  ...
  console.log(
    "[INFO] [CREATE_ORGANIZATION] Paid plan selected, starting Stripe checkout processing.",
    {
  ...
  const [planName, modalityName] = subscription.split("-") as [
    keyof typeof AppSubscriptionPlans,
    "MONTHLY" | "YEARLY",
  ];
  ...
  console.log(
    "[INFO] [CREATE_ORGANIZATION] Stripe customer created successfully with ID:",
    stripeCustomer.id,
  );
  ...
  console.log(
    "[INFO] [CREATE_ORGANIZATION] Stripe checkout session created successfully with URL:",
    checkoutSession.url,
  );
  ...
  try {
    ...
    console.error(
      "[WARN] [CREATE_ORGANIZATION] Falha ao capturar evento onboarding_completed_with_plan:",
      error,
    );
  }
...
async function updateOrganization({
  ...
  input: TUpdateOrganizationInput;
  session: TAuthUserSession;
}) {
  const userOrgId = session.membership?.organizacao.id;
  if (!userOrgId)
  ...
  console.log(
    "[INFO] [UPDATE_ORGANIZATION] Updating organization:",
  ...
  if (!updatedOrganizationId)
    throw new createHttpError.InternalServerError(
      "Oops, houve um erro desconhecido ao atualizar organização.",
    );
