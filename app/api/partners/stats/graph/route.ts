...
const GetPartnersGraphInputSchema = z.object({
	graphType: z.enum(["sales-value", "sales-quantity", "active-partners", "average-ticket"], {
...
async function getPartnersGraph({ input, sessionUser }: { input: TGetPartnersGraphInput; sessionUser: TAuthUserSession["user"] }) {
	...
	console.log(`[ORG: ${userOrgId}] [INFO] [GET PARTNERS GRAPH] Period:`, period);
	...
	// Graph Type: active-partners (unique partners who sold per period)
	if (input.graphType === "active-partners") {
		const activePartners = await db
			.select({
				date: sql<string>`date_trunc('day', ${sales.dataVenda})::text`,
				total: countDistinct(sales.parceiroId),
			})
			.from(sales)
			.where(
				and(
		...
		const initialActivePartners = Object.fromEntries(periodDatesStrs.map((date) => [dayjs(date).format(groupingFormat), { value: 0 }]));

		const activePartnersReduced = activePartners.reduce((acc, current) => {
			const saleDate = new Date(current.date);
			const saleTime = saleDate.getTime();
			...
			return acc;
		}, initialActivePartners);

		const activePartnersGraph = Object.entries(activePartnersReduced).map(([key, value]) => ({
			label: key,
			value: value.value,
		}));

		return {
			data: activePartnersGraph,
		};
	}
...
export type TGetPartnersGraphOutput = Awaited<ReturnType<typeof getPartnersGraph>>;

const getPartnersGraphRoute = async (request: NextRequest) => {
	const session = await getCurrentSessionUncached();
	if (!session) throw new createHttpError.Unauthorized("Você não está autenticado.");

	const searchParams = await request.nextUrl.searchParams;
	const input = GetPartnersGraphInputSchema.parse({
		graphType: searchParams.get("graphType") as "sales-value" | "sales-quantity" | "active-partners" | "average-ticket" | null,
