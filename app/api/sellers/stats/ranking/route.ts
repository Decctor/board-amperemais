...
async function fetchRankingForPeriod({
	...
	const saleConditions = [eq(sales.organizacaoId, userOrgId), isNotNull(sales.dataVenda), isNotNull(sales.vendedorId), eq(sales.statusVenda, "CONFIRMADA")];
...
async function getSellersRanking({ input, session }: { input: TGetSellersRankingInput; session: TAuthUserSession }) {
...
const getSellersRankingRoute: PagesRouteHandler<TGetSellersRankingOutput> = async (req, res) => {
	...
	const data = await getSellersRanking({ input, session: sessionUser });
