	...
	return (
		<ResponsiveMenuSection title="IMPORTAÇÃO DE PRODUTOS (OPCIONAL)" icon={<FileSpreadsheet className="w-4 h-4" />}>
			<div className="flex flex-col gap-3">
				...
				{productsExcelFile ? (
					<div className="bg-primary/5 border border-border rounded-lg p-4 flex items-center justify-between">
						<div className="flex items-center gap-3">
							<FileSpreadsheet className="w-8 h-8 text-green-600" />
							<div>
								<p className="text-sm font-medium">{productsExcelFile.name}</p>
								<p className="text-xs text-foreground/60">{(productsExcelFile.size / 1024).toFixed(2)} KB</p>
							</div>
						</div>
					...
					<label
						className="border-2 border-dashed border-border hover:border-border rounded-lg p-8 cursor-pointer transition-colors"
					...
					>
						<div className="flex flex-col items-center justify-center gap-3">
							...
							<p className="text-sm text-foreground/60 text-center">Clique para selecionar arquivo Excel</p>
							<p className="text-xs text-foreground/40 text-center">Formatos aceitos: .xlsx, .xls</p>
						</div>
						<input
							accept=".xlsx,.xls"
							className="hidden"
