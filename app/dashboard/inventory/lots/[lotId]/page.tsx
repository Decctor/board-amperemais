...
export default async function StockLotDetailPage({ params }: StockLotDetailPageProps) {
	const access = await requireDashboardCapability("inventory");
	...
	const stockLot = await db.query.productStockLots.findFirst({
		where: and(eq(productStockLots.id, lotId), eq(productStockLots.organizacaoId, sessionUser.membership.organizacao.id)),
		with: {
	...
	const printHref = `${appRoutes.inventory.lotLabelsPreview()}?ids=${encodeURIComponent(stockLot.id)}`;
			...
			<div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
				<div className="flex min-w-0 gap-3">
					<Button variant="outline" size="icon-sm" asChild>
						<Link href={appRoutes.inventory.lots()} aria-label="Voltar para lotes">
