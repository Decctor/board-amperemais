...
import { createMessageTemplate, submitMessageTemplate } from "@/lib/mutations/message-templates";
	...
	return (
		<div className="flex w-full flex-col gap-4">
			<div className="w-full flex flex-col gap-3  lg:flex-row lg:items-center lg:justify-between">
				<Button variant="ghost" size="fit" asChild className="rounded-full flex items-center gap-1 px-2 py-1">
					<Link href={`/dashboard/communication`} className="flex items-center gap-1">
									...
									{state.messageTemplate.conteudo.corpo.parametros.map((parametro, index) => (
										<div key={parametro.identificadorInterno} className="grid gap-2 rounded-lg bg-background p-2 md:grid-cols-[1fr_1fr]">
											...
											<Input
												...
												onChange={(event) => updateTemplateContentBodyParameter(index, { exemplo: event.target.value })}
												placeholder="Exemplo"
											/>
										</div>
									))}
