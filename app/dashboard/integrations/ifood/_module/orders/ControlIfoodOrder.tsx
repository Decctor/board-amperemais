		...
		>
			<ResponsiveMenu.Content drawerClassName="max-h-[70dvh]">
				<ResponsiveMenu.Header>
					<ResponsiveMenu.Title>{pedido?.displayId ? `PEDIDO #${pedido.displayId}` : "PEDIDO IFOOD"}</ResponsiveMenu.Title>
					<ResponsiveMenu.Description>
						Detalhes do pedido no iFood. As ações são enviadas para a API do iFood e o status definitivo chega pelos eventos.
					</ResponsiveMenu.Description>
				</ResponsiveMenu.Header>
				<ResponsiveMenuAnimatedBody
					stateKey={detailsQuery.isLoading ? "loading" : "content"}
					className={detailsQuery.isLoading ? "items-center justify-center p-6" : "overflow-x-hidden overflow-y-auto"}
				>
											...
											<div className="flex flex-wrap gap-2">
												{lifecycleActions.map((action) => (
													<Button
														...
														variant="outline"
														disabled={isPending}
														onClick={() => mutate({ orderId, action, cancellationCode: null })}
													>
				...
				<ResponsiveMenu.Footer>
					...
					<LoadingButton loading={isPending || detailsQuery.isLoading} onClick={closeModal}>
						FECHAR
					</LoadingButton>
				</ResponsiveMenu.Footer>
