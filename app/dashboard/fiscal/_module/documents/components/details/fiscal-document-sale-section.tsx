...
export function FiscalDocumentSaleSection({ sale }: FiscalDocumentSaleSectionProps) {
	...
	return (
		...
		>
			...
			{sale.itens.length > 0 ? (
				<>
					<div className="divide-y divide-border/70 border-t border-border px-4 sm:px-5">
						{sale.itens.map((item) => (
					...
					{sale.valorTotal != null ? (
						<div className="flex items-center justify-between border-t border-border bg-secondary/40 px-4 py-2.5 sm:px-5">
				...
				<div className="border-t border-border">
					<DetailsEmptyLine>Nenhum item no snapshot da venda.</DetailsEmptyLine>
				</div>
			)}
		</DetailsSection>
	);
