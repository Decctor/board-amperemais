...
function DetailField({ label, value, mono = false }: { label: string; value: string | null | undefined; mono?: boolean }) {
	return (
		<div className="flex flex-col gap-0.5">
			...
			<p className={cn("text-sm font-medium break-all", mono && "font-mono text-xs")}>{value?.trim() ? value : "—"}</p>
		</div>
	);
}

function TaxMetric({ label, value, formatBRL }: { label: string; value: number | null; formatBRL: (value: number | null | undefined) => string }) {
	return (
		<div className="rounded-lg border bg-muted/20 px-3 py-2">
			<p className="text-[10px] font-extrabold uppercase tracking-[0.08em] text-muted-foreground">{label}</p>
			<p className="mt-0.5 text-sm font-bold tabular-nums">{value != null ? formatBRL(value) : "—"}</p>
		</div>
	);
}
	...
	return (
		<Collapsible open={open} onOpenChange={setOpen} className="rounded-lg border">
			<div className="flex items-center justify-between gap-2 px-3 py-2">
				...
				<Button type="button" variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={copyJson}>
...
function TaxTotalsSection({ totals, formatBRL }: { totals: TFiscalDocumentTaxTotalsView; formatBRL: (value: number | null | undefined) => string }) {
	return (
		<ResponsiveMenuSection title="TOTAIS DE IMPOSTOS" icon={<Percent className="h-4 w-4" />}>
			<div className="grid grid-cols-2 gap-2 md:grid-cols-3">
				<TaxMetric label="Valor NF" value={totals.vNF} formatBRL={formatBRL} />
				<TaxMetric label="Produtos" value={totals.vProd} formatBRL={formatBRL} />
				<TaxMetric label="Desconto" value={totals.vDesc} formatBRL={formatBRL} />
				...
				<TaxMetric label="COFINS" value={totals.vCOFINS} formatBRL={formatBRL} />
				<TaxMetric label="Tributos (Lei 12.741)" value={totals.vTotTrib} formatBRL={formatBRL} />
			</div>
...
export function FiscalDocumentDetailsContent({
	...
	const mensagens = Array.isArray(document.mensagens)
		? document.mensagens.map((message) => (typeof message === "string" ? message : JSON.stringify(message)))
		: [];

	return (
		<div className="flex w-full flex-col gap-4">
			<ResponsiveMenuSection title="IDENTIFICAÇÃO" icon={<Receipt className="h-4 w-4" />}>
				<div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
					...
					<DetailField label="Ambiente" value={environmentLabels[document.ambiente] ?? document.ambiente} />
					<DetailField label="Provedor" value={document.provedor} />
					<DetailField label="Status SEFAZ" value={statusLabels[document.status] ?? document.status} />
					<DetailField label="Status interno" value={lifecycleStatusLabels[document.statusInterno] ?? document.statusInterno} />
					<DetailField label="ID no provedor" value={document.provedorDocumentoId} mono />
					<DetailField label="Chave de acesso" value={document.chaveAcesso} mono />
					<DetailField label="Protocolo" value={document.protocolo} mono />
					<DetailField label="Tentativas de envio" value={String(document.tentativasEnvio ?? 0)} />
					<DetailField label="Emissão" value={document.dataEmissao ? formatDateAsLocale(document.dataEmissao) : null} />
					<DetailField label="Autorização" value={document.dataAutorizacao ? formatDateAsLocale(document.dataAutorizacao) : null} />
					<DetailField label="Cancelamento" value={document.dataCancelamento ? formatDateAsLocale(document.dataCancelamento) : null} />
					<DetailField
						label="Última sincronização"
						value={document.dataUltimaSincronizacao ? formatDateAsLocale(document.dataUltimaSincronizacao) : null}
					/>
				</div>
				{document.documentoOrigem ? (
					<div className="rounded-lg border border-dashed bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
						Documento derivado de {document.documentoOrigem.tipo} Nº {document.documentoOrigem.numero ?? "—"}
						{document.chaveAcessoReferencia ? ` · ref. ${document.chaveAcessoReferencia}` : null}
					</div>
				) : null}
			</ResponsiveMenuSection>
				...
				<div className="rounded-lg border border-amber-500/60 bg-amber-50/70 p-4 dark:border-amber-800/70 dark:bg-amber-950/30">
					<div className="flex items-start gap-3">
						...
						<div className="min-w-0 flex-1 space-y-3">
							<div>
								<p className="text-sm font-bold text-amber-900 dark:text-amber-200">CLASSIFICAÇÃO PRESENCIAL EXCEPCIONAL</p>
							...
							<div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
								<DetailField label="Modalidade da venda" value={document.venda?.entregaModalidade ?? null} />
								<DetailField label="Presença declarada" value="OPERAÇÃO PRESENCIAL" />
								<DetailField
									label="Data da declaração"
									value={document.dataDeclaracaoPresencaConsumidor ? formatDateAsLocale(document.dataDeclaracaoPresencaConsumidor) : null}
								/>
								<DetailField label="Responsável" value={document.autorPresencaConsumidor?.nome ?? null} />
							</div>
							<div>
			...
			{saleSummary ? (
				<ResponsiveMenuSection title="VENDA VINCULADA" icon={<ShoppingCart className="h-4 w-4" />}>
					<div className="flex flex-wrap items-start justify-between gap-3">
						<div className="grid flex-1 grid-cols-1 gap-3 sm:grid-cols-2">
							<DetailField label="Cliente" value={saleSummary.clienteNome} />
							<DetailField label="CPF/CNPJ" value={saleSummary.clienteCpfCnpj} mono />
							<DetailField label="Data da venda" value={saleSummary.dataVenda ? formatDateAsLocale(saleSummary.dataVenda) : null} />
							<DetailField label="Valor total" value={saleSummary.valorTotal != null ? formatBRL(saleSummary.valorTotal) : null} />
							<DetailField label="Status da venda" value={saleSummary.statusVenda} />
					...
					{saleSummary.itens.length > 0 ? (
								...
								<TableBody>
									{saleSummary.itens.map((item) => (
										<TableRow key={item.id}>
											...
											<TableCell className="text-right tabular-nums">{formatBRL(item.valorTotal)}</TableCell>
										</TableRow>
									))}
								</TableBody>
							</Table>
						...
						<p className="text-sm text-muted-foreground">Nenhum item encontrado no snapshot da venda.</p>
					)}
				</ResponsiveMenuSection>
			) : null}

			{taxTotals ? <TaxTotalsSection totals={taxTotals} formatBRL={formatBRL} /> : null}

			{payloadItems.length > 0 ? (
				<ResponsiveMenuSection title="ITENS DO PAYLOAD FISCAL" icon={<Package className="h-4 w-4" />}>
					<div className="rounded-lg border">
						<Table>
							<TableHeader>
								<TableRow>
							...
							<TableBody>
								{payloadItems.map((item) => (
									<TableRow key={item.numero}>
										<TableCell className="tabular-nums">{item.numero}</TableCell>
										...
										<TableCell className="font-mono text-xs">{item.csosn ?? "—"}</TableCell>
										<TableCell className="text-right tabular-nums">{item.quantidade ?? "—"}</TableCell>
										<TableCell className="text-right tabular-nums">{item.valorTotal != null ? formatBRL(item.valorTotal) : "—"}</TableCell>
									</TableRow>
								))}
							</TableBody>
						</Table>
			...
			{(providerPayload || providerResponse) && (
				<ResponsiveMenuSection title="PAYLOAD E RETORNO" icon={<FileJson className="h-4 w-4" />}>
					<div className="flex flex-col gap-2">
						<JsonPanel title="Payload enviado ao provedor" value={providerPayload} />
						<JsonPanel title="Retorno do provedor" value={providerResponse} />
					</div>
					{!providerPayload ? (
						<p className="text-xs text-muted-foreground">
							O payload ainda não foi gerado. Ele aparece após a primeira tentativa de envio ao provedor fiscal.
						</p>
					) : null}
				</ResponsiveMenuSection>
			)}
			...
			{mensagens.length > 0 ? (
				<ResponsiveMenuSection title="MENSAGENS" icon={<Hash className="h-4 w-4" />}>
					<div className="flex flex-col gap-1.5">
						{mensagens.map((message, index) => (
							<p key={`${index}-${message.slice(0, 24)}`} className="rounded-md bg-muted/30 px-3 py-2 text-xs leading-relaxed text-foreground/90">
								{message}
							</p>
						))}
					</div>
			...
			<ResponsiveMenuSection title="HISTÓRICO DE EVENTOS" icon={<History className="h-4 w-4" />}>
				{events.length > 0 ? (
					<div className="flex flex-col gap-2">
						{events.map((event) => (
							<div key={event.id} className="rounded-lg border bg-muted/20 px-3 py-2">
								<div className="flex flex-wrap items-center justify-between gap-2">
									<p className="text-xs font-bold tracking-tight">{FISCAL_EVENT_TYPE_LABELS[event.tipo as TFiscalDocumentEventTypeEnum] ?? event.tipo}</p>
									<p className="text-[11px] text-muted-foreground">{formatDateAsLocale(event.dataInsercao)}</p>
								</div>
