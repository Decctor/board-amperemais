...
import { FiscalDocumentActionBar } from "./_components/fiscal-document-action-bar";
import { FiscalDocumentActionsDropdown } from "./_components/fiscal-document-actions-dropdown";
import { FiscalDocumentNextSteps } from "./_components/fiscal-document-next-steps";
import { FiscalProblemChips } from "./_components/fiscal-document-problem-chips";
import { FiscalPendingView } from "./_components/fiscal-pending-view";
import { isFiscalDocumentFailed, type TFiscalPermissions } from "./_components/fiscal-document-action-state";
import { useFiscalDocumentActionRunner } from "./_components/use-fiscal-document-action-runner";
...
export default function FiscalPage({
	...
	const [viewMode, setViewMode] = useQueryState("view", parseAsStringEnum(["pending", "documents", "configuration"]));
	const [, setSelectedDocumentId] = useQueryState("documentId");
	...
	const activeView = viewMode ?? (pendingTotal > 0 ? "pending" : "documents");
	const openDocument = (documentId: string) => {
		void setViewMode("documents");
		void setSelectedDocumentId(documentId);
	};
	return (
		<div className="w-full h-full flex flex-col gap-3">
			<Tabs value={activeView} onValueChange={(v) => setViewMode(v as typeof viewMode)}>
...
function buildFiscalDocumentCardTitle(document: TGetFiscalDocumentsOutputDefault["documents"][number]) {
...
function buildFiscalDocumentCardSubtitle(document: TGetFiscalDocumentsOutputDefault["documents"][number]) {
...
function buildFiscalDocumentStatusTooltip(document: TGetFiscalDocumentsOutputDefault["documents"][number]) {
...
function FiscalDocumentStatusIcon({ statusInterno }: { statusInterno: TFiscalDocumentLifecycleStatusEnum }) {
	...
	return (
		<TooltipProvider>
			...
			>
				<div className="flex w-full items-start justify-between gap-3">
					...
					<div className="flex shrink-0 items-center gap-1.5" onClick={(event) => event.stopPropagation()}>
						...
						<StatBadge
							...
							value={FISCAL_LIFECYCLE_STATUS_LIST_LABELS[document.statusInterno]}
				...
				{isErrored ? (
					<FiscalDocumentCardProblems document={document} userHasFiscalConfigurePermission={userHasFiscalConfigurePermission} openDetails={openDetails} />
				) : null}
			</div>
		</TooltipProvider>
	);
...
function FiscalDocumentDetailsMenu({
	...
	const { data, isLoading, isError, error, refetch } = useFiscalDocumentById(documentId);
...
function FiscalDocumentDetailsBody({
	document,
	events,
	permissions,
	exceptionalPresenceEnabled,
				...
				<div className="flex flex-wrap items-center justify-between gap-2">
					<div className="flex flex-col gap-1">
						...
						<p className="text-xs text-muted-foreground">
							...
							{isFiscalDocumentFailed(document.statusInterno) && document.codigoRejeicao ? ` · Rejeição ${document.codigoRejeicao}` : ""}
						</p>
					</div>
				</div>
...
function FiscalConfigurationsView({ userHasFiscalConfigurePermission }: FiscalConfigurationsViewProps) {
	...
	// `?section=` (vindo de uma CTA de problema) rola ate o bloco certo depois que os dados chegam.
...
function ExceptionalPresenceClassificationSettings({ fiscalConfig, updateFiscalConfig, disabled }: ExceptionalPresenceClassificationSettingsProps) {
...
function FiscalEnvironmentSwitcher({ fiscalConfig, updateFiscalConfig, disabled }: FiscalEnvironmentSwitcherProps) {
...
function InboundDfeSettings({ fiscalConfig, updateFiscalConfig }: InboundDfeSettingsProps) {
...
function CompanyBasicInformation({ fiscalConfig, updateFiscalConfig, callbacks }: CompanyBasicInformationProps) {
	...
	return (
		<SectionWrapper title="EMPRESA FISCAL" icon={<Receipt className="h-4 w-4" />} wrapperClassName="scroll-mt-4">
...
const FISCAL_LIFECYCLE_STATUS_LIST_LABELS: Record<TFiscalDocumentLifecycleStatusEnum, string> = {
	RASCUNHO: "RASCUNHO",
