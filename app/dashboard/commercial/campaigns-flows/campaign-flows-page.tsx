    ...
    <div className="w-full h-full flex flex-col gap-3">
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="flex items-center gap-1.5 w-fit h-fit self-start rounded-lg px-2 py-1">
          <TabsTrigger value="flows" className="flex items-center gap-1.5 px-2 py-2 rounded-lg">
            <GitBranch className="w-4 h-4 min-w-4 min-h-4" />
            Campanhas
          </TabsTrigger>
          <TabsTrigger value="audiences" className="flex items-center gap-1.5 px-2 py-2 rounded-lg">
        ...
        <TabsContent value="audiences" className="flex flex-col gap-3">
          <AudiencesTab />
        </TabsContent>
      </Tabs>
    </div>
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <Button size="sm" className="flex items-center gap-1.5" asChild>
          <Link href="/dashboard/commercial/campaigns-flows/canvas">
          ...
          <div className="flex flex-col gap-2">
            ...
            {flowsResult.campaignFlows.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-8">
                Nenhum fluxo de campanha encontrado.
              </p>
            )}
          </div>
...
function CampaignFlowCard({ flow, onEdit, onDelete, isDeleting }: CampaignFlowCardProps) {
  ...
  const typeConfig = FLOW_TYPE_CONFIG[flow.tipo] ?? {
    label: flow.tipo,
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-lg border p-3 hover:bg-muted/50 transition-colors">
      <div className="flex flex-col gap-1 min-w-0 flex-1">
        ...
        {audience.descricao && (
          <p className="text-xs text-muted-foreground truncate">{audience.descricao}</p>
        )}
        <div className="flex items-center gap-3 mt-1">
