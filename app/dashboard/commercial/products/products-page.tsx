  ...
  return (
    <div className="w-full h-full flex flex-col gap-3">
      <Tabs
        ...
        onValueChange={(v: string) => setViewMode(v as "stats" | "database")}
      >
  ...
  return (
    <div className="w-full flex flex-col gap-3">
      ...
      <GeneralPaginationComponent
        ...
        itemsMatchedText={
          productsMatched > 0
            ? `${productsMatched} produtos encontrados.`
            : `${productsMatched} produto encontrado.`
        }
        itemsShowingText={
          productsShowing > 0
            ? `Mostrando ${productsShowing} produtos.`
            : `Mostrando ${productsShowing} produto.`
        }
      />
...
function ProductsStatsView() {
  ...
  const { data: productsOverallStats, isLoading: productsOverallStatsLoading } =
    useProductsOverallStats({
        ...
        <DateIntervalInput
          ...
          handleChange={(value) =>
            setFilters((prev) => ({
              ...
              comparingPeriodAfter: value.after
                ? dayjs(value.after).subtract(1, "month").toDate()
                : null,
              comparingPeriodBefore: value.before
                ? dayjs(value.before).subtract(1, "month").toDate()
                : null,
            }))
          }
        />
...
function ProductCard({
  ...
  return (
    <div
      className={cn(
        "bg-card border-primary/20 flex w-full flex-col sm:flex-row gap-2 rounded-xl border px-3 py-4 shadow-2xs",
      )}
    >
                      ...
                      <StatBadge
                        ...
                        className={cn({
                          ...
                          "bg-yellow-500 dark:bg-yellow-600 text-white":
                            turnoverDays >= 7 && turnoverDays < 30,
                          "bg-green-500 dark:bg-green-600 text-white":
                            turnoverDays >= 30 && turnoverDays < 90,
                          "bg-blue-500 dark:bg-blue-600 text-white":
                            turnoverDays >= 90 && turnoverDays < 180,
        ...
        <div className="w-full flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center gap-1.5">
            <div className="flex items-center gap-1">
              ...
              <h1 className="py-0.5 text-center text-[0.65rem] font-medium italic text-primary/80">
                {product.precoVenda
                  ? `${formatToMoney(product.precoVenda)} / ${product.unidade}`
                  : "PREÇO DE VENDA NÃO DEFINIDO"}
              </h1>
            </div>
          </div>
          <div className="flex items-center gap-1.5">
      ...
      {filters.stockStatus && filters.stockStatus.length > 0 && (
        <FilterTag
          ...
          value={filters.stockStatus
            .map(
              (status: string) =>
                STOCK_STATUS_MAP[status as keyof typeof STOCK_STATUS_MAP] || status,
            )
            .join(", ")}
