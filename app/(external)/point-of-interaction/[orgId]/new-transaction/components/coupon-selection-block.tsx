...
export function CouponSelectionBlock({ coupons, isLoading, selectedCouponId, onSelect, onClear }: CouponSelectionBlockProps) {
	...
	return (
		<div className="bg-card rounded-3xl short:rounded-xl p-6 short:p-3 border-2 border-brand/5 shadow-sm flex flex-col gap-4 short:gap-2 animate-in fade-in slide-in-from-bottom-4">
			<div className="flex items-center gap-3 short:gap-1.5">
				...
				<h3 className="font-black uppercase italic short:text-xs">Cupons Disponíveis</h3>
			</div>
				...
				{coupons.map((coupon) => {
					...
					return (
						<button
							...
							className={cn(
								"w-full flex items-center justify-between gap-3 short:gap-1.5 rounded-2xl short:rounded-lg border-2 short:border px-4 py-3 short:px-2.5 short:py-2 text-left transition-all",
						...
						>
							<div className="flex flex-col min-w-0">
								<span className="font-black text-sm short:text-xs uppercase tracking-tight truncate">
