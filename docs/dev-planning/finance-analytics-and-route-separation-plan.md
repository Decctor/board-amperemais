...
- [x] Route separation completed
- [x] Analytics backend completed
- [x] Analytics UI completed
...
- [x] Use routes for distinct jobs; tabs only for alternate views within the same context.
- [x] Make every route-level workspace independently linkable and refresh-safe.
- [x] Clean cutover: no redirects, no legacy `?view=` shims, update every internal link, remove
      the old tab contract in the same migration.
- [x] Portuguese labels in the UI, English names in routes and code.
- [x] Portuguese field names in API payloads (see *Payload language* below).
- [x] Keep the API authoritative for authorization; sidebar visibility is not a security boundary.
- [x] Avoid redesigning the CRUD content of existing tabs while moving them.
...
- [x] `Visão geral` (the ported executive summary) lives at the module root, mirroring how sales
      history lives at `/dashboard/sales`. It replaces the current `stats` tab.
- [x] `Relatórios` is a single sidebar child covering the three analyses, keeping the Financeiro
...
- [x] `Faturas de cartão` keeps its own route (1:1 with the current tab). Alternative considered
      and rejected for now: folding it into `accounts` as local navigation.
- [x] Plano de contas stays in Settings (`SettingsFinances`). The DRE depends on the
...
- [x] Recurring rules (`financial_recurring_rules`) still have no dedicated surface; they remain
...
- [x] Move `stats` (default tab) to `/dashboard/finance` (upgraded to the executive summary).
- [x] Move `accounting-entries` to `/dashboard/finance/entries`.
- [x] Move `financial-transactions` to `/dashboard/finance/transactions`.
- [x] Move `financial-accounts` to `/dashboard/finance/accounts`.
- [x] Move `credit-card-invoices` to `/dashboard/finance/credit-cards`.
- [x] Move `reconciliation` to `/dashboard/finance/reconciliation`.
- [x] Add `/dashboard/finance/reports/income-statement`.
- [x] Add `/dashboard/finance/reports/cash-flow`.
- [x] Add `/dashboard/finance/reports/receivables-payables`.
- [x] Remove the `view` query-state contract from the finance page.
- [x] Do not add redirects or keep a compatibility `?view=` reader.
...
- [x] Convert `appRoutes.finance` into a nested builder group
...
- [x] Update the existing `appRoutes.finance()` call site in `components/Sidebar/AppSidebar.tsx`.
- [x] Search for `"/dashboard/finance"` literals repo-wide and replace with builders.
- [x] Search for `view=stats`, `view=accounting-entries`, `view=financial-transactions`,
      `view=financial-accounts`, `view=credit-card-invoices`, `view=reconciliation` and resolve
      every result.
- [x] Extend `lib/navigation/routes.test.ts` with the new builders.

## Sidebar

- [x] Convert the flat `finance` item in `SidebarConfig` (group `Gestão`) into a parent with
      `url: appRoutes.finance.root()`, prefix match, and the seven children above.
- [x] Give `Visão geral` `activeMatch: "exact"` so it does not swallow sibling routes (same
      technique as `sales-history`).
- [x] Give `Relatórios` prefix match on `/dashboard/finance/reports`.
- [x] Keep every child on the existing `finance` capability for this migration (see access model).
...
- [x] Every new `page.tsx` calls `requireDashboardCapability("finance")`.
- [x] Resolve and pass granular props per route from `lib/permissions/finances.ts`:
      `canCreateFinances` / `canEditFinances` where the workspace mutates, and
      `canReconcileFinances` for the reconciliation route.
- [x] Reports routes are read-only; no mutation props.
- [x] Do not read `permissoes.financeiro` outside `lib/permissions/finances.ts`.
...
- [x] `classification.ts` — revenue/cost/expense classifier. **Adaptation:** RecompraCRM's
...
- [x] Use this single classifier for the executive summary and DRE. (Control's
...
- [x] `tree.ts` — DRE tree roll-up: per-account totals bubble to ancestors (max depth 20),
      zero-value nodes pruned, children sorted desc.
- [x] `periods.ts` — previous-period window (immediately preceding window of identical duration),
      12-month series bucketing.
- [x] `cash.ts` — consolidated position (`totalCaixa`, `totalPassivos`,
...
- [x] `aging.ts` — six buckets (VENCIDO +60 / 31-60 / 1-30, A VENCER 0-30 / 31-60 / +60),
...
- [x] Exclude transfers everywhere flow is measured: `metodo !== "TRANSFERENCIA"` (and entries
      with `origemTipo === "TRANSFERENCIA"` where aggregating the accrual side).
- [x] Scope every query by `organizacaoId` (Control scopes by `parceiroId`; the tenancy key
      changes, the shape does not).
- [x] Accrual vs cash split is preserved exactly: the DRE aggregates
...
- [x] Credit-card forecast dates reuse the existing `lib/finances/credit-card.ts` helpers.
- [x] No balance checkpoints: compute current balances from `saldoInicial` + transaction sums
      (existing account-balance helpers). Record checkpoint tables/crons as a performance
      follow-up, out of scope here.
- [x] Unit tests per file: classification over a natureza tree with partner-created sub-accounts,
...
- [x] `analytics/dre/route.ts` — `TGetFinancesDreOutput`: current + previous period totals,
...
- [x] `analytics/cash-flow/route.ts` — multi-mode GET: `visaoGeral` (position, burn, runway,
      recurring commitments, saldo por conta) and `projecao` (`horizonteDias` 7–180, default 90,
      daily series, `primeiraDataNegativa`).
- [x] `analytics/receivables-payables/route.ts` — aging buckets, totals + overdue, average
      delays, friction (`jurosMultasPagos`, `descontosObtidos`, `taxasPorMetodo`), liquidity
      block (capital de giro, cobertura de fixos).
- [x] Extend `app/api/finances/stats/route.ts` with previous-period totals so the executive
...
- [x] All routes authenticated + `canViewFinances` enforced server-side.
- [x] Query hooks in `lib/queries/finances.ts` (or a new `lib/queries/finance-analytics.ts`),
...
- [x] Move each `finances-page-*.tsx` view into its route directory, renamed to the
      `{feature}-page.tsx` convention. Moving ≠ redesigning: CRUD behavior is untouched.
- [x] While moving, split the two oversized views per the target architecture:
      `finances-page-reconciliation.tsx` (548 l.) and `finances-page-financial-accounts.tsx`
      (440 l.) get local `_components/`.
- [x] Port the four kept Control sections as the new pages, adapted to RecompraCRM primitives:
...
- [x] Port `DeltaBadge` into `_components/` (period-over-period badge with `invert` semantics for
      cost/expense lines).
- [x] Each route owns its loading, error, and empty states.
- [x] Each route exports page `metadata`.
- [x] Keep useful local query state (filters, pagination) per route; drop only the `view`
      contract.
- [x] Modals stay where they are (`components/Modals/Finances/**`, `AccountingEntries/**`) for
...
- [x] Route registry conversion + call-site updates + tests.
- [x] Scaffold the six workspace routes; move the six existing views.
- [x] Sidebar group with children, capability filtering, active-state matching.
- [x] Per-route server permission resolution.
- [x] Delete the old tab shell (`finances-page.tsx` Tabs + `view` contract).
- [x] Link audit (`/dashboard/finance` literals, `view=` literals).

### Phase 2 — Analytics backend

- [x] `lib/finances/analytics/` library + unit tests.
- [x] `analytics/dre`, `analytics/cash-flow`, `analytics/receivables-payables` routes.
- [x] `stats` route previous-period extension.
- [x] Query hooks.

### Phase 3 — Analytics UI

- [x] Reports scaffolding + local secondary navigation.
- [x] DRE workspace (statement lines, drill-down trees, 12-month composed chart).
- [x] Cash-flow workspace (position KPIs, projection charts with synced tooltips,
      negative-balance alert).
- [x] Receivables & payables workspace (aging chart, delay/friction cards).
- [x] Overview upgrade to executive summary with deltas.

### Phase 4 — Cleanup and cross-doc updates

- [x] Remove unused imports/utilities left by the moves.
- [x] Update `dashboard-information-architecture-and-routes-plan.md`: finance local-tabs line
      superseded, follow-up recorded, finance page/API alignment noted.
- [x] Update `financial-module-improvements-plan.md`: Phase 3 dashboard-totals items
      cross-referenced to the cash-flow workspace.

## Verification

- [x] `npm run lint`, `npm run format:check`, focused tests, `npm run build`. (Lint 0 errors; format
      clean on every touched file — the repo-wide check fails on pre-existing files; build compiles,
      with page-data collection/export limited by missing env secrets and DB in the sandbox.)
- [x] Route-builder and analytics unit tests pass.
...
- [x] DRE totals reconcile with the executive summary totals for the same period (single
      classifier guarantee).
- [ ] Transfers excluded: a transfer between accounts changes no report flow totals.
- [x] No `view=` or `/dashboard/finance` string literals remain outside the registry.
