...
export const ClientSchema = z.object({
  ...
  dataUltimaCompra: z.string({ invalid_type_error: 'Tipo não válido para data da última compra.' }),
  idUltimaCompra: z.string({ invalid_type_error: 'Tipo não válido para data da última compra.' }),
  analiseRFM: z.object({
    notas: z.object({
      recencia: z.number(),
      frequencia: z.number(),
...
export type TClientSimplifiedWithSales = TClientSimplified & { vendas: Pick<TSale, 'valor' | 'dataVenda'>[] }
export type TClientSimplifiedWithSalesDTO = TClientSimplifiedWithSales & { _id: string }
export const ClientSearchQueryParams = z.object({
  ...
  rfmTitles: z.array(z.string({ invalid_type_error: 'Tipo não válido para filtro por título RFM.' })),
  period: z.object({
