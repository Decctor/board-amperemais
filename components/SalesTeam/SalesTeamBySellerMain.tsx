...
type SalesTeamBySellerMainProps = {
	id: string;
	session: TUserSession;
};
export default function SalesTeamBySellerMain({ id, session }: SalesTeamBySellerMainProps) {
			...
			<div className="flex h-full flex-col">
				...
				<div className="flex w-full max-w-full grow flex-col overflow-x-hidden bg-background px-6 lg:px-12 py-6 gap-6">
					<div className="flex w-full items-center justify-between border-b border-primary pb-2 gap-2">
						<h1 className="text-base text-center lg:text-start lg:text-2xl font-black text-primary">Resultados do Vendedor</h1>
				...
				{editSellerMenuIsOpen ? <EditSeller sellerId={id} sessionUser={session} closeModal={() => setEditSellerMenuIsOpen(false)} /> : null}
			</div>
		);
	return <></>;
}
