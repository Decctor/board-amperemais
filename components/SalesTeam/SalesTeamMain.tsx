...
import { BadgeDollarSign, CirclePlus, Filter, Mail, Pencil, Phone } from "lucide-react";
			...
			<div className="flex w-full max-w-full grow flex-col overflow-x-hidden bg-background px-6 lg:px-12 py-6 gap-6">
				...
				{isSuccess ? (
					<div className="w-full flex flex-col gap-1.5">
						{sellers.length > 0 ? (
							sellers.map((seller) => <SellerCard key={seller.id} seller={seller} handleEditClick={setEditSellerId} />)
						) : (
...
function SellerCard({ seller, handleEditClick }: SellerCardProps) {
	return (
