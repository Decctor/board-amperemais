...
import { Lightbulb, RefreshCw, Sparkles, WandSparkles, X } from "lucide-react";
...
export function AIHintsBadge() {
	...
	const hasHints = !isDismissed && hintsCount > 0;
		...
		<>
			...
			<Popover open={isOpen} onOpenChange={setIsOpen}>
				<PopoverTrigger asChild>
					<Button
						variant={hasHints ? "brand" : "ghost"}
						size="sm"
						className={cn(
							"flex items-center gap-1.5 rounded-full px-3 font-semibold text-xs tracking-wide transition-all duration-300",
							hasHints && "shadow-sm",
						)}
					>
						<Sparkles className="h-3.5 w-3.5" />
						<span className="hidden sm:inline">DICAS IA</span>
						{hasHints && (
							<span className="flex h-4 w-4 items-center justify-center rounded-full bg-white/20 text-[10px] font-bold leading-none">
								{hintsCount}
							</span>
						)}
					</Button>
				...
				>
							...
							<div className="flex gap-1">
								{!isDismissed && (
									<Button
										variant="ghost"
										size="sm"
										className="h-7 rounded-full px-2 text-[10px] text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
					...
					<div className="flex flex-col gap-1 px-2 py-3 flex-1 min-h-0 overflow-y-auto scrollbar-thin scrollbar-thumb-secondary scrollbar-track-transparent">
						{isDismissed ? (
							<div className="flex flex-col items-center justify-center py-8 px-4 text-center gap-3">
								...
								<p className="text-sm text-muted-foreground">Você ocultou as dicas da IA.</p>
								<Button
									size="sm"
									variant="outline"
							...
							<div className="flex flex-col items-center justify-center py-10 px-5 text-center">
								...
								<p className="mt-1 text-xs text-muted-foreground">
									Isso pode levar alguns instantes enquanto a IA analisa seus dados.
								</p>
					...
					<div className="border-t border-border/50 bg-secondary/40 px-4 py-3">
						...
						<p className="mt-2 text-center text-[10px] text-muted-foreground">
							Dicas geradas por IA com base nos dados do seu negócio
						</p>
					</div>
				</PopoverContent>
			</Popover>
