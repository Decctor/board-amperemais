...
import { TClientSimplifiedWithSalesDTO } from '@/schemas/clients'
...
function ClientCard({ client, period }: ClientCardProps) {
  ...
  function getSalesStatsInPeriod(sales: TClientSimplifiedWithSalesDTO['vendas']) {
    const salesInPeriod = sales.filter((sale) => new Date(sale.dataVenda) >= period.after && new Date(sale.dataVenda) <= period.before)
    ...
    <div className="flex w-full flex-col gap-1 rounded border border-primary bg-[#fff] p-2 shadow-sm dark:bg-[#121212]">
      <div className="w-full flex items-center justify-between gap-2 flex-col lg:flex-row">
        <div className="flex items-center gap-2">
          ...
          <h1 className={cn('px-2 py-0.5 rounded-lg text-white text-[0.6rem]', getRFMColor(client.analiseRFM.titulo))}>{client.analiseRFM.titulo}</h1>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1">
            <BsCalendar width={10} height={10} />
            <h1 className="py-0.5 text-center text-[0.6rem] font-medium italic text-primary/80">ÚLTIMA COMPRA</h1>
            <h1 className="py-0.5 text-center text-[0.6rem] font-bold  text-primary">{formatDateAsLocale(client.dataUltimaCompra) || 'N/A'}</h1>
          </div>
          <div className="flex items-center gap-1">
            <BsCalendar width={10} height={10} />
            <h1 className="py-0.5 text-center text-[0.6rem] font-medium italic text-primary/80">PRIMEIRA COMPRA</h1>
            <h1 className="py-0.5 text-center text-[0.6rem] font-bold  text-primary">{formatDateAsLocale(client.dataPrimeiraCompra) || 'N/A'}</h1>
          </div>
        </div>
      </div>
