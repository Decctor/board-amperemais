				...
				<div className="w-full flex flex-col grow gap-2 py-2">
					...
					<div className="w-full lg:w-1/2 flex flex-col rounded border border-primary shadow-sm self-center">
						...
						<p className="text-center px-6 mb-2 mt-4 text-xs font-medium italic text-primary/80">
							Os números representam uma análise de matriz RFM nos últimos 12
							meses. Os dados são atualizados diariamente.
						</p>
								...
								{rfmStats?.map((item, index) => (
									...
									>
										{item.rfmLabel !== "PERDIDOS (extensão)" ? (
											<h1 className="text-[0.4rem] lg:text-base">
												{item.rfmLabel}
											</h1>
										) : (
											""
										...
										{item.rfmLabel !== "PERDIDOS (extensão)" ? (
											<div className="bg-black h-5 w-5 min-h-5 min-w-5 lg:h-16 lg:w-16 lg:min-h-16 lg:min-w-16 p-2 rounded-full flex items-center justify-center">
												<h1 className="text-[0.4rem] lg:text-sm font-bold text-white">
													{item.clientsQty}
												</h1>
											</div>
										) : (
			...
			<div className="w-full flex items-center justify-center lg:justify-end px-2 flex-wrap-reverse lg:flex-wrap gap-2">
				...
				{queryParams.rfmTitles.length > 0 ? (
					<div className="flex items-center gap-1">
						...
						<h1 className="py-0.5 text-center text-[0.6rem] font-bold  text-primary">
							{queryParams.rfmTitles.length > 0
								? queryParams.rfmTitles.map((title) => title).join(", ")
								: "N/A"}
						</h1>
					</div>
				) : null}
					...
					clients.length > 0 ? (
						clients.map((client, index: number) => (
							<ClientCard
								...
								client={client}
								period={{
									after: queryParams.period.after
										? new Date(queryParams.period.after)
										: new Date(),
...
type ClientCardProps = {
	client: TClientsBySearch["clients"][number];
...
function ClientCard({ client, period }: ClientCardProps) {
	...
	function getSalesStatsInPeriod(
		sales: TClientSimplifiedWithSalesDTO["vendas"],
	) {
		const salesInPeriod = sales.filter(
			(sale) =>
				new Date(sale.dataVenda) >= period.after &&
				new Date(sale.dataVenda) <= period.before,
		);
		const totalValue = salesInPeriod.reduce((acc, sale) => acc + sale.valor, 0);
		return { purchases: salesInPeriod.length, purchasesTotal: totalValue };
	}
	const salesStats = getSalesStatsInPeriod(client.vendas);
	return (
		<div className="flex w-full flex-col gap-1 rounded border border-primary bg-[#fff] p-2 shadow-sm dark:bg-[#121212]">
			<div className="w-full flex items-center justify-between gap-2 flex-col lg:flex-row">
				<div className="flex items-center gap-2">
					<h1 className="text-[0.6rem] font-bold tracking-tight lg:text-sm">
						{client.nome}
					</h1>
					<h1
						className={cn(
							"px-2 py-0.5 rounded-lg text-white text-[0.6rem]",
							getRFMColor(client.analiseRFM.titulo),
						)}
					>
						{client.analiseRFM.titulo}
					</h1>
				</div>
				<div className="flex items-center gap-2 flex-wrap">
					<div className="flex items-center gap-1">
						...
						<h1 className="py-0.5 text-center text-[0.6rem] font-bold  text-primary">
							{formatDateAsLocale(client.dataUltimaCompra) || "N/A"}
						</h1>
					</div>
					<div className="flex items-center gap-1">
						<BsCalendar width={10} height={10} />
						...
						<h1 className="py-0.5 text-center text-[0.6rem] font-bold  text-primary">
							{formatDateAsLocale(client.dataPrimeiraCompra) || "N/A"}
						</h1>
					</div>
				</div>
			</div>
			<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
				...
				<div className="flex w-full flex-wrap items-center justify-center gap-2 lg:min-w-fit lg:justify-end">
					...
					<div className="flex items-center gap-1">
						...
						<h1 className="py-0.5 text-center text-[0.6rem] font-bold  text-primary">
							{formatToMoney(salesStats.purchasesTotal)}
						</h1>
					</div>
				</div>
			</div>
