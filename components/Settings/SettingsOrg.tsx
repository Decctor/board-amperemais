...
import {
  type TUseOrganizationState,
  useOrganizationState,
} from "@/state-hooks/use-organization-state";
  ...
  if (!permissions.visualizar) {
    return (
      <div className="flex w-full flex-col items-center justify-center p-8 text-center text-muted-foreground">
        ...
        <p className="text-sm">
          Você não tem permissão para visualizar as configurações da organização.
        </p>
      </div>
    );
  }
...
function SettingsOrgContent({ userHasEditPermissions }: SettingsOrgContentProps) {
  ...
  const {
    state,
    updateOrganization: updateOrgState,
    updateLogoHolder,
    redefineState,
  } = useOrganizationState();
      ...
      redefineState({
        ...
        organization: {
          ...
          corPrimariaForeground:
            organization.corPrimariaForeground || DEFAULT_ORG_COLORS.primaryForeground,
          corSecundaria: organization.corSecundaria || DEFAULT_ORG_COLORS.secondary,
          corSecundariaForeground:
            organization.corSecundariaForeground || DEFAULT_ORG_COLORS.secondaryForeground,
          configuracao: {
            ...state.organization.configuracao,
            preferencias: {
              ...state.organization.configuracao.preferencias,
  ...
  const primaryForegroundColor =
    state.organization.corPrimariaForeground || DEFAULT_ORG_COLORS.primaryForeground;
  const secondaryColor = state.organization.corSecundaria || DEFAULT_ORG_COLORS.secondary;
  const secondaryForegroundColor =
    state.organization.corSecundariaForeground || DEFAULT_ORG_COLORS.secondaryForeground;
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between border-b pb-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Gerencie as informações gerais, identidade visual e preferências da sua organização.
          </p>
        </div>
            ...
            <div className="relative group">
              <div className="relative h-32 w-32 overflow-hidden rounded-xl border-2 border-dashed border-muted-foreground/25 bg-muted/50 transition-colors hover:bg-muted">
                {state.organization.logoUrl ? (
                  ...
                  <Image
                    src={state.logoHolder.previewUrl}
                    alt="Logo da organização"
                    fill
                    className="object-cover"
                  />
          ...
          <div className="flex-1 grid gap-4">
            ...
            <div className={"flex w-full flex-col gap-1"}>
              <h3 className={cn("text-sm font-medium tracking-tight text-primary/80")}>
                LIMITE DE ENVIOS DE MENSAGEM VIA CAMPANHA (POR SEMANA)
              </h3>
              ...
              <div className="w-full flex items-center justify-start flex-wrap gap-x-2 gap-y-1">
                <Button
                  variant={
                    !state.organization.configuracao.preferencias
                      .limiteMensagensSemanaisViaCampanhas
                      ? "default"
                      : "ghost"
                  }
                  size={"fit"}
                ...
                <Button
                  variant={
                    state.organization.configuracao.preferencias
                      .limiteMensagensSemanaisViaCampanhas === 300
                      ? "default"
                      : "ghost"
                  }
                  size={"fit"}
                ...
                <Button
                  variant={
                    state.organization.configuracao.preferencias
                      .limiteMensagensSemanaisViaCampanhas === 500
                      ? "default"
                      : "ghost"
                  }
                  size={"fit"}
                ...
                <Button
                  variant={
                    state.organization.configuracao.preferencias
                      .limiteMensagensSemanaisViaCampanhas === 1000
                      ? "default"
                      : "ghost"
                  }
                  size={"fit"}
                  ...
                  onClick={() =>
                    updateOrgState({
                      configuracao: {
                        ...
                        preferencias: {
                          ...state.organization.configuracao.preferencias,
                          limiteMensagensSemanaisViaCampanhas: 1500,
                        },
                      },
                    })
                  }
                  ...
                  handleChange={(value) =>
                    updateOrgState({
                      configuracao: {
                        ...
                        preferencias: {
                          ...state.organization.configuracao.preferencias,
                          limiteMensagensSemanaisViaCampanhas: value,
                        },
                      },
                    })
                  }
              ...
              <div className="flex items-center gap-3">
                <div className="relative h-10 w-10 overflow-hidden rounded-lg border shadow-sm">
                  <input
                    ...
                    value={primaryColor}
                    onChange={(e) =>
                      userHasEditPermissions && updateOrgState({ corPrimaria: e.target.value })
                    }
              ...
              <div className="flex items-center gap-3">
                <div className="relative h-10 w-10 overflow-hidden rounded-lg border shadow-sm">
                  <input
                    ...
                    onChange={(e) =>
                      userHasEditPermissions &&
                      updateOrgState({ corPrimariaForeground: e.target.value })
                    }
                    disabled={!userHasEditPermissions}
                    className="absolute -inset-2 h-[200%] w-[200%] cursor-pointer"
                  />
              ...
              <div className="flex items-center gap-3">
                <div className="relative h-10 w-10 overflow-hidden rounded-lg border shadow-sm">
                  <input
                    ...
                    onChange={(e) =>
                      userHasEditPermissions && updateOrgState({ corSecundaria: e.target.value })
                    }
                    disabled={!userHasEditPermissions}
                    className="absolute -inset-2 h-[200%] w-[200%] cursor-pointer"
                  />
              ...
              <div className="flex items-center gap-3">
                <div className="relative h-10 w-10 overflow-hidden rounded-lg border shadow-sm">
                  <input
                    ...
                    onChange={(e) =>
                      userHasEditPermissions &&
                      updateOrgState({ corSecundariaForeground: e.target.value })
                    }
                    disabled={!userHasEditPermissions}
                    className="absolute -inset-2 h-[200%] w-[200%] cursor-pointer"
                  />
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              <span className="text-xs text-muted-foreground">
                VALORES DE ESTATÍSTICAS (COR SECUNDÁRIA)
              </span>
              <div
