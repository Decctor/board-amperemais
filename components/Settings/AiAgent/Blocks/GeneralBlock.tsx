...
type GeneralBlockProps = {
	...
	updateModeloConfig: TUseInternalAiAgentState["updateModeloConfig"];
};

/** Identidade, estado e modelo. Um agente PAUSADO segue editável, mas não é executado. */
export default function GeneralBlock({ state, updateAgent, updateModeloConfig }: GeneralBlockProps) {
	const ativo = state.agente.status === "ATIVO";
	const temperatura = state.agente.modeloConfig.temperatura ?? 1;
	...
	return (
		<div className="flex w-full flex-col gap-4">
			<div className="flex w-full flex-col gap-1 rounded-lg border border-border bg-card px-4 py-3">
				<div className="flex items-center justify-between gap-4">
					<div className="flex flex-col">
						<h3 className="text-sm font-bold tracking-tight">{ativo ? "AGENTE ATIVO" : "AGENTE PAUSADO"}</h3>
					...
					<Switch checked={ativo} onCheckedChange={(checked) => updateAgent({ status: checked ? "ATIVO" : "PAUSADO" })} />
				</div>
			</div>
			...
			<SelectInput
				...
				handleChange={(value) => updateModeloConfig({ modelo: value })}
				onReset={() => updateModeloConfig({ modelo: DEFAULT_AI_AGENT_MODEL })}
			...
			<div className="flex w-full flex-col gap-2">
				<div className="flex items-center justify-between">
					...
					<span className="text-sm font-bold text-primary">{temperatura.toFixed(1)}</span>
				</div>
				<input
					type="range"
					min={0}
					...
					step={0.1}
					value={temperatura}
					onChange={(event) => updateModeloConfig({ temperatura: Number(event.target.value) })}
