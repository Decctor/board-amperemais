...
export default function KnowledgeBlock({ state, addKnowledgeBlock, updateKnowledgeBlock, removeKnowledgeBlock }: KnowledgeBlockProps) {
	...
	const visibleBlocks = state.conhecimento.map((block, index) => ({ block, index })).filter(({ block }) => !block.deletar);
	...
	return (
		<div className="flex w-full flex-col gap-4">
			...
			{visibleBlocks.map(({ block, index }) => (
				<div key={block.id ?? `novo-${index}`} className="flex w-full flex-col gap-3 rounded-lg border border-border bg-card px-4 py-3">
					<div className="flex items-center justify-between gap-4">
						...
						<div className="flex items-center gap-3 pt-6">
							<Switch
								...
								aria-label={`Ativar bloco ${block.titulo || index + 1}`}
							/>
