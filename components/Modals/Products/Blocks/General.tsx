...
export default function ProductGeneralBlock({
  ...
  return (
    <ResponsiveMenuSection
      ...
      icon={<LayoutGrid className="h-4 min-h-4 w-4 min-w-4" />}
    >
...
function ProductGeneralBlockImage({
  ...
  return (
    <div className="flex items-center justify-center min-h-[250px] min-w-[250px]">
      <label
        className="relative aspect-square w-full max-w-[250px] cursor-pointer overflow-hidden rounded-lg"
...
function UsersGeneralBlockAvatarPreview({
  ...
  if (imageHolder.previewUrl) {
    return (
      <Image alt="Capa do produto." fill={true} objectFit="cover" src={imageHolder.previewUrl} />
    );
  }
  if (imageUrl) {
