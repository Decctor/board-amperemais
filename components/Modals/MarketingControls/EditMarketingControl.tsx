...
import { useMarketingControlById } from "@/lib/queries/marketing-controls";
import { updateMarketingControl } from "@/lib/mutations/marketing-controls";
...
type EditMarketingControlProps = {
	marketingControlId: string;
	session: TUserSession;
	closeModal: () => void;
};
function EditMarketingControl({ marketingControlId, session, closeModal }: EditMarketingControlProps) {
	const queryClient = useQueryClient();

	const { data: marketingControl, isLoading, isError, isSuccess, error } = useMarketingControlById(marketingControlId);
	...
	const { mutate, isPending } = useMutationWithFeedback({
		mutationKey: ["update-marketing-control", marketingControlId],
		mutationFn: updateMarketingControl,
		queryClient: queryClient,
		affectedQueryKey: ["marketing-controls"],
		callbackFn: async () => queryClient.invalidateQueries({ queryKey: ["marketing-control-by-id", marketingControlId] }),
	});
	console.log(infoHolder);
	useEffect(() => {
		if (marketingControl) setInfoHolder(marketingControl);
	}, [marketingControl]);
	return (
			...
			<Dialog.Content className="fixed left-[50%] top-[50%] z-[100] h-[80%] w-[90%] translate-x-[-50%] translate-y-[-50%] rounded-md bg-background p-[10px] lg:h-[80%] lg:w-[40%]">
				<div className="flex h-full w-full flex-col">
					<div className="flex flex-col items-center justify-between border-b border-gray-200 px-2 pb-2 text-lg lg:flex-row">
						<h3 className="text-sm font-bold lg:text-xl">EDITAR CONTROLE DE MARKETING</h3>
						<button
							onClick={() => closeModal()}
							type="button"
						...
						<>
							...
							<div className="mt-1 flex w-full items-end justify-end">
								<LoadingButton
									onClick={() => {
										// @ts-ignore
										mutate({ id: marketingControlId, changes: infoHolder });
									}}
									loading={isPending}
								>
									ATUALIZAR CONTROLE
...
export default EditMarketingControl;
