...
export default function ManualFiscalEmission({
  ...
  return (
    <ResponsiveMenu
      ...
      menuActionButtonVariant={
        mode === "PRESENCIAL_EXCEPCIONAL" ? "destructive" : "default"
      }
      menuActionButtonDisabled={
        mode === "PRESENCIAL_EXCEPCIONAL" && !exceptionalModeIsValid
      }
      ...
      <div className="space-y-4">
        ...
        >
          <CheckCircle2
            className={cn(
              ...
              mode === "REGULAR" ? "text-primary" : "text-muted-foreground",
            )}
          />
          <div>
            <p className="text-sm font-semibold">
              Usar a natureza real da venda
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Opção recomendada. Vendas com entrega serão informadas como
              operação não presencial e exigem CPF ou CNPJ válido do
              destinatário.
            </p>
          </div>
        </button>
        ...
        {canOfferExceptionalMode ? (
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold">
                Declarar como presencial excepcionalmente
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                Use somente sob orientação contábil específica. A venda
                continuará registrada como entrega e esta declaração será
                auditada.
              </p>
            </div>
          </button>
        ...
        {mode === "PRESENCIAL_EXCEPCIONAL" ? (
          <div className="space-y-4 rounded-lg border border-destructive/40 bg-destructive/5 p-4">
            <div className="flex items-start gap-2 text-sm">
              ...
              <div>
                <p className="font-semibold text-destructive">
                  Evite esta classificação sempre que puder identificar o
                  destinatário.
                </p>
                <p className="mt-1 text-muted-foreground">
                  A autorização do documento não comprova que a declaração
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <Label htmlFor="exceptional-presence-justification">
              ...
              <p className="text-xs text-muted-foreground">
                Mínimo de {EXCEPTIONAL_PRESENCE_JUSTIFICATION_MIN_LENGTH}{" "}
                caracteres. O texto será salvo no histórico fiscal.
              </p>
            </div>
            <div className="flex items-start gap-3 rounded-lg border border-destructive/30 bg-background/70 p-3">
              <Checkbox
                id="confirm-exceptional-presence"
                checked={acknowledged}
                onCheckedChange={(checked) => setAcknowledged(checked === true)}
              />
              <Label
                htmlFor="confirm-exceptional-presence"
                className="cursor-pointer text-sm font-normal leading-5"
              >
                Confirmo que esta venda possui entrega, mas estou instruindo o
                sistema a informar presença física do consumidor nesta
                tentativa.
              </Label>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 rounded-lg border bg-muted/20 p-3 text-xs text-muted-foreground">
            ...
            <span>
              O sistema usará o perfil fiscal correspondente ao canal e à
              modalidade registrados na venda.
            </span>
          </div>
        )}
      </div>
