...
import { createGoal, updateGoal } from '@/lib/mutations/goals'
...
type EditSaleGoalProps = {
  goalId: string
  session: TUserSession
  closeModal: () => void
}
function EditSaleGoal({ goalId, session, closeModal }: EditSaleGoalProps) {
  const queryClient = useQueryClient()
  const { data: goal, isLoading, isError, isSuccess, error } = useGoalById(goalId)
  ...
  const { mutate, isPending } = useMutationWithFeedback({
    ...
    callbackFn: async () => queryClient.invalidateQueries({ queryKey: ['goal-by-id', goalId] }),
  })

  useEffect(() => {
    if (goal) setInfoHolder(goal)
  }, [goal])
  return (
      ...
      <Dialog.Content className="fixed left-[50%] top-[50%] z-[100] h-[80%] w-[90%] translate-x-[-50%] translate-y-[-50%] rounded-md bg-background p-[10px] lg:h-[80%] lg:w-[40%]">
        <div className="flex h-full w-full flex-col">
          <div className="flex flex-col items-center justify-between border-b border-gray-200 px-2 pb-2 text-lg lg:flex-row">
            <h3 className="text-sm font-bold lg:text-xl">EDITAR META</h3>
            <button
              onClick={() => closeModal()}
              type="button"
            ...
            <>
              ...
              <div className="mt-1 flex w-full items-end justify-end">
                <LoadingButton
                  onClick={() => {
                    // @ts-ignore
                    mutate({ id: goalId, changes: infoHolder })
                  }}
                  loading={isPending}
                >
                  ATUALIZAR META
