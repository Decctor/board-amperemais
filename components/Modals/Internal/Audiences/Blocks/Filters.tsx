...
type AudienceFiltersBlockProps = {
	filtros: TUseInternalAudienceState["state"]["filtros"];
	segmentacoes: TUseInternalAudienceState["state"]["segmentacoes"];
	addFilterCondition: TUseInternalAudienceState["addFilterCondition"];
	updateFilterCondition: TUseInternalAudienceState["updateFilterCondition"];
	addFilterGroup: TUseInternalAudienceState["addFilterGroup"];
	updateFilterGroupOperator: TUseInternalAudienceState["updateFilterGroupOperator"];
	removeFilterNode: TUseInternalAudienceState["removeFilterNode"];
};
...
export default function AudienceFiltersBlock({
	...
	removeFilterNode,
}: AudienceFiltersBlockProps) {
	const [editor, setEditor] = useState<EditorState>(null);
	const normalizedFilters = normalizeAudienceFiltersForSubmit(filtros);
	const {
		data: preview,
		isLoading: isPreviewLoading,
		isError: isPreviewError,
	} = useQuery({
		queryKey: ["audience-preview", segmentacoes, normalizedFilters],
		queryFn: () => previewAudience({ segmentacoes, filtros: normalizedFilters }),
		enabled: segmentacoes.length > 0 || !!normalizedFilters,
	});
	...
	return (
		<ResponsiveMenuSection title="FILTROS AVANÇADOS" icon={<Filter className="h-4 w-4" />}>
			<p className="text-xs italic text-muted-foreground">Combine filtros com árvores AND/OU/NÃO para refinar o público além das segmentações.</p>
			...
			<div className="flex items-center justify-between gap-2 rounded-md border border-border bg-primary/5 px-3 py-2">
				<div className="flex items-center gap-2">
					...
					<p className="text-xs font-semibold uppercase tracking-wide text-foreground/80">Prévia do público</p>
				</div>
				{isPreviewLoading ? (
					<AnimatedSpinner className="h-4 min-h-4 w-4 min-w-4" />
				) : isPreviewError ? (
					<span className="text-sm text-destructive">Erro ao calcular o público.</span>
				) : segmentacoes.length === 0 && !normalizedFilters ? (
					<span className="text-xs italic text-muted-foreground">Selecione segmentações ou filtros</span>
				) : (
					<p className="text-sm">
						<strong className="tabular-nums">{(preview?.totalClients ?? 0).toLocaleString("pt-BR")}</strong> cliente(s)
					</p>
				)}
			</div>
					...
					return (
						<button
							...
							className={cn(
								...
								active ? "border-border bg-primary text-primary-foreground" : "border-border text-foreground/70 hover:bg-primary/10",
...
function TopBuyersDetails({ condicao }: { condicao: Extract<TCampaignFilterCondition, { tipo: "TOP_COMPRADORES_PRODUTO" }> }) {
	...
	return (
		<span className="truncate text-sm">
			Top {top} compradores de <strong>{isLoading ? "carregando..." : (product?.descricao ?? "produto não encontrado")}</strong>{" "}
