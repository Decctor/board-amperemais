...
import { Code, Plus, Search, ShoppingCart, Trash2 } from "lucide-react";
	...
	return (
		<ResponsiveMenuSection title="ITENS" icon={<ShoppingCart className="h-4 min-h-4 w-4 min-w-4" />}>
			...
			<div className="w-full flex flex-col gap-1.5">
				{items.length > 0 ? (
					items.map((item, index) => <ItemCard key={item.produtoId} item={item} handleRemoveClick={() => deleteItem(index)} />)
				) : (
...
function ItemCard({ item, handleRemoveClick }: ItemCardProps) {
	return (
		<div className={cn("bg-card border-primary/20 flex w-full flex-col gap-1 rounded-xl border px-3 py-2 shadow-2xs")}>
			...
			<div className="w-full flex items-center justify-between">
				...
				<Button variant="ghost" className="flex items-center gap-1.5" size="sm" onClick={handleRemoveClick}>
