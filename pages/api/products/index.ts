...
const GetProductsDefaultInputSchema = z.object({
  ...
  priceMax: z
    ...
    .transform((val) => (val ? Number(val) : null)),
  orderByField: z
    .enum(["descricao", "codigo", "grupo", "vendasValorTotal", "vendasQtdeTotal", "quantidade"])
    .optional()
    .nullable(),
...
async function getProducts({ input, session }: GetProductsParams) {
  ...
  if ("id" in input) {
    ...
    const product = await db.query.products.findFirst({
      where: (fields, { and, eq }) =>
        and(eq(fields.id, input.id), eq(fields.organizacaoId, userOrgId)),
    });
  ...
  if (input.statsSaleNatures && input.statsSaleNatures.length > 0)
    statsConditions.push(inArray(sales.natureza, input.statsSaleNatures));
  if (input.statsExcludedSalesIds && input.statsExcludedSalesIds.length > 0)
    statsConditions.push(notInArray(sales.id, input.statsExcludedSalesIds));
  if (input.statsSellerIds && input.statsSellerIds.length > 0)
    statsConditions.push(inArray(sales.vendedorId, input.statsSellerIds));
  ...
  switch (input.orderByField) {
    ...
    case "vendasValorTotal":
      orderByClause = direction(
        sql`COALESCE(sum(CASE WHEN ${sales.id} IS NOT NULL THEN ${saleItems.valorVendaTotalLiquido} ELSE 0 END), 0)`,
      );
      break;
    case "vendasQtdeTotal":
      orderByClause = direction(
        sql`COALESCE(sum(CASE WHEN ${sales.id} IS NOT NULL THEN ${saleItems.quantidade} ELSE 0 END), 0)`,
      );
      break;
    case "quantidade":
  ...
  const productsWithStatsResult = await baseQuery
...
async function updateProduct({
  ...
  if (!updatedProductId)
    throw new createHttpError.InternalServerError(
      "Oops, houve um erro desconhecido ao atualizar produto.",
    );
  return {
    data: {
...
const CreateProductAddOnOptionInputSchema = ProductAddOnOptionSchema.omit({
  organizacaoId: true,
  produtoAddOnId: true,
}).extend({
...
async function createProduct({
  ...
  const transactionReturn = await db.transaction(async (tx) => {
    ...
    if (!createdProduct?.id)
      throw new createHttpError.InternalServerError("Erro ao criar o produto.");

    const productId = createdProduct.id;

    // 1.1 Checking if product allows stock tracking and if quantity is greater than 0
    if (
      input.product.rastreamentoEstoqueAtivo &&
      input.product.quantidade &&
      input.product.quantidade > 0
    ) {
    ...
    for (const variant of input.productVariants) {
      ...
      for (const [addOnIndex, addOn] of variant.addOns.entries()) {
        ...
        if (!createdAddOn?.id)
          throw new createHttpError.InternalServerError(
            "Erro ao criar grupo de adicionais da variante.",
          );
    ...
    for (const [addOnIndex, addOn] of input.productAddOns.entries()) {
      ...
      if (!createdAddOn?.id)
        throw new createHttpError.InternalServerError(
          "Erro ao criar grupo de adicionais do produto.",
        );
