...
import { SalesGeneralStatsFiltersSchema, type TSalesGeneralStatsFilters } from "@/schemas/query-params-utils";
...
const getSalesDashboardStatsRoute: NextApiHandler<{ data: TGeneralSalesStats }> = async (req, res) => {
	...
	const stats = sales.reduce(
		(acc: TSalesReduced, current) => {
			...
			for (const item of current.itens.filter((item) => (productGroups.length > 0 ? productGroups.includes(item.grupo) : true))) {
...
type TSaleResult = {
	...
	clienteDados: {
		analiseRFM: {
			titulo: TClient["analiseRFMTitulo"];
		};
	};
};
