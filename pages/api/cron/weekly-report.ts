	...
	try {
		...
		for (const organization of organizationsList) {
			try {
				...
				console.log(`[ORG: ${organization.id}] [INFO] [WEEKLY_REPORT] Report data prepared:`, {
					periodo,
					faturamento,
					meta,
					percentualMeta,
				...
				for (const recipient of REPORT_RECIPIENTS) {
					try {
						console.log(`[ORG: ${organization.id}] [INFO] [WEEKLY_REPORT] Sending report to ${recipient}`);
						...
						console.log(`[ORG: ${organization.id}] [INFO] [WEEKLY_REPORT] Successfully sent report to ${recipient}`);
					} catch (error) {
						console.error(`[ORG: ${organization.id}] [ERROR] [WEEKLY_REPORT] Failed to send report to ${recipient}:`, error);
						allResults.push({
							organizationId: organization.id,
							recipient,
							status: "error",
