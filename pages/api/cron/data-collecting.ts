...
import { clients, saleItems, sales, type TNewSaleItemEntity } from "@/services/drizzle/schema";
...
const handleOnlineSoftwareImportation: NextApiHandler<string> = async (req, res) => {
	...
	return await db.transaction(async (tx) => {
		...
		for (const OnlineSale of OnlineSoftwareSales) {
			...
			const saleItemsToInsert: TNewSaleItemEntity[] = OnlineSale.itens.map((item) => {
				const equivalentProduct = existingProductsMap.get(item.codigo);
				if (!equivalentProduct) {
					throw new createHttpError.InternalServerError(`Produto ${item.codigo} não encontrado.`);
				}
			...
			if (saleItemsToInsert.length > 0) await tx.insert(saleItems).values(saleItemsToInsert);
		}
